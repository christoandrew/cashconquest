...
import (
	...
	"github.com/christo-andrew/maybe-go/internal/api/schemas"
	...
	"github.com/christo-andrew/maybe-go/pkg/pagination"
...
func GetAccountTransactionsHandler(c *gin.Context, db *gorm.DB) {
	...
	transactions := scopes.GetAccountTransactions(db, accountId)
...
func GetRecentTransactionsHandler(c *gin.Context, db *gorm.DB) {
	...
	scopes.GetRecentTransactions(db, accountId, 5).Find(&transactions)
